...
async function enhanceHauler(html: string, request: Request, env: any): Promise<string> {
  ...
  const extra = [
    ...
    '<div class="field full"><label for="business_address">Business address</label><input id="business_address" name="business_address" autocomplete="street-address" maxlength="180" value="'+escapeHtml(details?.business_address)+'" placeholder="Start typing your business address" required><input id="business_place_id" name="business_place_id" type="hidden" value="'+escapeHtml(details?.business_place_id)+'"><input name="business_lat" type="hidden" value="'+escapeHtml(details?.business_lat)+'"><input name="business_lng" type="hidden" value="'+escapeHtml(details?.business_lng)+'"><small style="color:#61778d">Google address confirmation will be required once the Maps key is connected.</small></div>',
    ...
    '<div class="field full"><label for="private_fill_stations">Private fill station(s)</label><textarea id="private_fill_stations" name="private_fill_stations" maxlength="600" placeholder="Enter the name and full address of any private fill station you use.">'+escapeHtml(details?.private_fill_stations)+'</textarea></div>',
