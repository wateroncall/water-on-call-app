...
const ACCOUNT_SCRIPT = [
  ...
  '      const progress = document.createElement("div"); progress.className = "order-progress"; if (order.status === "cancelled") { const cancelled = document.createElement("div"); cancelled.className = "cancelled-progress"; cancelled.textContent = "This request was cancelled."; progress.append(cancelled); } else { const stages = ["requested","offered","accepted","assigned","en_route","delivered"]; const current = Math.max(0, stages.indexOf(order.status)); stages.forEach((stage, index) => { const step = document.createElement("div"); step.className = index < current ? "progress-step complete" : index === current ? "progress-step current" : "progress-step"; const dot = document.createElement("span"); const text = document.createElement("small"); text.textContent = label(stage); step.append(dot, text); progress.append(step); }); } expanded.append(progress);',
