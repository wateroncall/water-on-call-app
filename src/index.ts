...
const ACCOUNT_SCRIPT = [
  ...
  '      const progress = document.createElement("div"); progress.className = "order-progress"; if (order.status === "cancelled") { const cancelled = document.createElement("div"); cancelled.className = "cancelled-progress"; cancelled.textContent = "Cancelled — this request is closed."; progress.append(cancelled); } else { const stages = ["requested","offered","accepted","assigned","en_route","delivered"]; const current = Math.max(0, stages.indexOf(order.status)); const heading = document.createElement("strong"); heading.className = "progress-heading"; heading.textContent = "Delivery progress"; progress.append(heading); stages.forEach((stage, index) => { const step = document.createElement("div"); step.className = index < current ? "progress-step complete" : index === current ? "progress-step current" : "progress-step upcoming"; const icon = document.createElement("span"); icon.textContent = index < current ? "✓" : index === current ? "•" : "○"; const text = document.createElement("div"); const name = document.createElement("b"); const state = document.createElement("small"); name.textContent = label(stage); state.textContent = index < current ? "Completed" : index === current ? "Current status" : index === current + 1 ? "Next step" : "Upcoming"; text.append(name, state); step.append(icon, text); progress.append(step); }); } expanded.append(progress);',
...
function accountPage(user: AccountUserRow): Response {
  const html = [
    ...
    'button{height:49px;width:100%;border:0;border-radius:11px;background:var(--blue);color:#fff;font-size:15px;font-weight:800;cursor:pointer;margin-top:17px}button:disabled{opacity:.6;cursor:not-allowed}.fine{font-size:12px;margin-top:10px}.order-list{display:grid;gap:11px}.order-item{border:1px solid var(--line);border-radius:12px;padding:14px;cursor:pointer}.order-item:hover,.order-item:focus{border-color:var(--blue);outline:none;box-shadow:0 0 0 3px #0877f914}.order-top{display:flex;justify-content:space-between;gap:10px}.order-item p{font-size:13px;margin-top:5px}.view-hint{display:inline-block;margin-top:8px;color:var(--blue);font-size:12px;font-weight:750}.order-details{border-top:1px solid var(--line);margin-top:12px;padding-top:12px;cursor:default}.order-details div{display:grid;grid-template-columns:125px 1fr;gap:10px;padding:6px 0;font-size:13px}.order-details strong{color:var(--navy)}.order-details span{color:var(--muted);overflow-wrap:anywhere}.order-progress{display:grid;gap:7px;margin-bottom:14px;background:#f8fbfe;border-radius:11px;padding:13px}.progress-heading{margin-bottom:2px;color:var(--navy)}.progress-step{display:grid;grid-template-columns:25px 1fr;gap:8px;align-items:center;padding:4px 0}.progress-step>span{display:grid;place-items:center;width:22px;height:22px;border-radius:50%;border:1px solid #c8d7e5;color:#91a2b3;font-weight:850}.progress-step b{display:block;font-size:13px;color:#7b8d9e}.progress-step small{display:block;font-size:10px;color:#91a2b3}.progress-step.complete>span{background:#e4f8ef;border-color:#a8dcc7;color:var(--green)}.progress-step.complete b{color:var(--green)}.progress-step.current{background:#eaf4ff;border-radius:8px;padding:7px}.progress-step.current>span{background:var(--blue);border-color:var(--blue);color:#fff}.progress-step.current b,.progress-step.current small{color:var(--navy)}.cancelled-progress{background:#fff0f0;color:var(--red);border-radius:9px;padding:10px;font-size:13px;font-weight:750}.repeat-button,.cancel-button{width:auto;height:42px;margin:12px 9px 0 0;padding:0 18px}.repeat-button{background:#eaf4ff;color:var(--navy)}.repeat-button:hover{background:#dbeeff}.cancel-button{background:#fff0f0;color:var(--red);border:1px solid #f2caca}.cancel-button:hover{background:#ffe4e4}.status{background:#eaf4ff;color:var(--navy);border-radius:999px;padding:5px 9px;font-size:11px;font-weight:800;white-space:nowrap}.empty{padding:18px;border:1px dashed #bdd0e1;border-radius:12px;text-align:center;font-size:14px}',
