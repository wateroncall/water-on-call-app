...
async function sendLaunchBatch(env: any, body: any) {
  ...
  const response = await fetch(RESEND_BATCH_ENDPOINT, {
    ...
    body: JSON.stringify([customerPayload(body), adminPayload(body)]),
  });

  const detail = await response.text();
  if (!response.ok) throw new Error(`Resend ${response.status}: ${detail}`);
  console.log("Launch email batch accepted by Resend", detail);
}
...
export default {
  async fetch(request: Request, env: any, ctx: ExecutionContext): Promise<Response> {
    ...
    if (request.method === "POST" && url.pathname === "/api/launch/waitlist") {
      ...
      try {
        // One Resend batch call contains BOTH messages. This prevents the admin notice
        // succeeding while the customer confirmation is silently skipped.
        await sendLaunchBatch(env, body);
      } catch (error: any) {
        const reason = String(error?.message || error || "Unknown email error");
        console.error("Launch email batch failed", reason);
        return new Response(JSON.stringify({
          error: `Your launch signup was saved, but the confirmation email could not be sent: ${reason}`
        }), {
          status: 502,
