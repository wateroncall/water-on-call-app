...
async function customerStatusNotification(env:any,order:any,hauler:any,status:string){
 ...
 await sendNotification(env,{eventType:`order.${status}.customer_notification`,channel:"email",recipient:order.customer_email,subject:`Water OnCall delivery update — ${label}`,html,orderId:order.id,userId:order.customer_id,metadata:{hauler_id:hauler.id,status}});
}
...
async function haulerTransition(req:Request,env:any,orderId:string){
 ...
 const order=await env.DB.prepare(`SELECT o.id,o.status,o.customer_id,o.gallons,o.city,cu.email AS customer_email FROM order_assignments a JOIN orders o ON o.id=a.order_id JOIN users cu ON cu.id=o.customer_id WHERE a.order_id=? AND a.hauler_id=? LIMIT 1`).bind(orderId,u.id).first<any>();
