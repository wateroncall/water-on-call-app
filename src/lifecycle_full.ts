...
async function customerStatusNotification(env:any,order:any,hauler:any,status:string){
 ...
 if(order.customer_email&&wantsEmail(order.notification_preference))await sendNotification(env,{eventType:`order.${status}.customer_notification`,channel:"email",recipient:order.customer_email,subject:`Water OnCall delivery update — ${label}`,html,orderId:order.id,userId:order.customer_id,metadata:{hauler_id:hauler.id,status}});
 if(order.customer_phone&&wantsSms(order.notification_preference))await sendNotification(env,{eventType:`order.${status}.customer_notification`,channel:"sms",recipient:order.customer_phone,text:`Water OnCall: Your ${Number(order.gallons||0).toLocaleString()} gallon delivery with ${String(hauler.business_name||"your hauler")} is now ${label}. Sign in for details.`,orderId:order.id,userId:order.customer_id,metadata:{hauler_id:hauler.id,status}});
}
...
async function haulerTransition(req:Request,env:any,orderId:string){
 ...
 const order=await env.DB.prepare(`SELECT o.id,o.status,o.customer_id,o.gallons,o.city,cu.email AS customer_email,cu.phone AS customer_phone,COALESCE(cp.notification_preference,'email') AS notification_preference FROM order_assignments a JOIN orders o ON o.id=a.order_id JOIN users cu ON cu.id=o.customer_id LEFT JOIN customer_profiles cp ON cp.user_id=cu.id WHERE a.order_id=? AND a.hauler_id=? LIMIT 1`).bind(orderId,u.id).first<any>();
